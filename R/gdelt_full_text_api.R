...
.extract_date_json <- function(x) {
  # Remove everything before the first parenthesis
  x <- substr(x, regexpr("\\(", x) + 1, nchar(x))
  x <-
    x |> str_split("\\,") |> flatten_chr() |> str_squish()
  x <- x[length(x)] |> str_remove_all("\\)|\\'")
  x <- x |> lubridate::mdy_hm(tz = 'UTC') %>%  lubridate::with_tz(Sys.timezone())
...
dictionary_stability_locations <-
  memoise::memoise(function(snake_names = F) {
    ...
    if (snake_names) {
      location_df <- location_df |> janitor::clean_names()
    }
    location_df
...
.read_codebook <-
  function(url = "http://data.gdeltproject.org/api/v2/guides/LOOKUP-GKGTHEMES.TXT" ,
           id_name = 'idGKGTheme') {
    data <- data.table::fread(url) |> as_tibble() |>set_names(c(id_name, 'value'))
